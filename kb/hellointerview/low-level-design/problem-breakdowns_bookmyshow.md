...
Bad Solution: Check then book (no synchronization)

Great Solution: Synchronized booking with atomic check-and-set
...
Good Solution: Fine-grained locking (per-seat)
...
Aren't we worried about multiple threads putting simultaneously in that hashmap? Should it be a concurrentHashMap ? Or Should the we wrap both showtime.book() and reservationsById.put inside a synchronized block
...
enhancement bug:  when narration is enabled. Clicking on some paragraph, sometimes reads out different paragraph than the one selected.
