...
Bad Solution: Skip the orchestrator, just expose the root Folder

Great Solution: FileSystem orchestrator owns path resolution
...
Bad Solution: Store the path as a string

Great Solution: Store a parent pointer
...
Bad Solution: List of children

Great Solution: Map from name to entry
...
Good Solution: Shared interface

Great Solution: Abstract base class
...
If create_file is called with a path ending with /, the code breaks I think (i checked python implementation).
Ex:_resolve_parent('/a/b/') -> '/a/b'

Then we call _extract_name('/a/b/') -> ''

We'll attempt to create an entry with an empty string which shouldn't be allowed
