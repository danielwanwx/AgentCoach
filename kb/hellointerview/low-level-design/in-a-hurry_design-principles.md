...
This is the single principle we see most often violated in low-level design interviews.
Candidates often over-engineer because they want to show off their knowledge of design patterns. They'll introduce factories, builders, and decorators when a basic class would work fine. Interviewers notice this. They want to see that you can distinguish between problems that need sophisticated solutions and problems that need simple ones.
The time to add complexity is when simplicity stops working. If your single class grows to 500 lines with ten different responsibilities, that's when you refactor. If adding a new payment method means modifying code in five places, that's when you introduce a strategy pattern. But start simple.
...
Instead, we define a MessageSender interface based on what NotificationService needs, and inject the implementation through the constructor.
Good: Follows DIP
Python
Language
from abc import ABC, abstractmethod
...
Now NotificationService depends on MessageSender (an abstraction), and EmailSender also depends on MessageSender (it implements the interface). Neither high-level nor low-level module knows about the other. You can swap email for SMS by passing a different implementation. You can unit test with a mock MessageSender that doesn't send real messages. The business logic is insulated from implementation details.
Note that DIP is a design principle, while dependency injection (passing dependencies through the constructor) is a technique for achieving it. They're related but not the same thing.
...
I agree. Toy examples may seem like they're easier to conceptualize, but then the reader has to draw a connection between the toy example and a real-world example. Why not just go straight to the real-world example to save the reader the mental hop.

When thinking about code, I find it more relatable to, for example, talk about a payment processor because its an actual system that software engineers implement. I've never had to implement a duck (although I do find ducks adorable).
...
So don't stress over memorizing acronyms or listing off SOLID like it's the alphabet. Interviewers care that you apply the lessons, not that you can name them.

So the exact opposite happened to me in Interviews in several Indian giants. My interviewer at the Indian eCommerce giant (you can probably guess this one) asked me to specifically name what patterns can be applied to a said class (he was only looking for Factory,  it wasn't an obvious clean fit in my opinion) and expected me to tick off each letter off of SOLID.

As someone coming from a functional programming background (read OCaml/Haskell and Rust), the LLD round make lesser sense to me then the general population. Based on my exp, I think they are highly opinionated and expect bookish knowledge. Tbh, it was quite a culture shock for me to see these LLD rounds (and MachineCoding where you're supposed to code and run the entire thing in 65 mins) exist at many places these days and is currently a bottleneck in my prep, I'm losing offers because of this single round.
...
Yeah I personally think the way these interviews are conducted sometimes is quite baffling, especially for engineers with a functional background as you mention.

We’ve done our best to toe the line between “what we think is reasonable” and “what people actually experience” but some poorly constructed interviews will scribble completely outside these lines.
...
I guess best I can do now is develop some intuition for Object Oriented design in general and just memorize how SOLID patterns are applied in each case :/
There's also a heavy emphasis on converting entity classes into a rigid data model.

PS - I'm anyway going for a HelloInterview Premium now, would request you guys to push out more LLD content. Rest of it is anyway top notch stuff.
...
Totally agree @sarthak soni . I have been in a few interviews where the expectation was to implement running end to end code within 45 minutes. India ecosystem as some bizarre requirements
...
When NotificationService depends on a concrete EmailSender, you can't unit test without sending real emails, and you can't swap to SMS without modifying the service

In your example, the real issue seems to be NotificationService constructing EmailSender directly (new EmailSender()), which makes it hard to substitute in tests.
But if EmailSender were injected via constructor (even as a concrete type), we could still unit test NotificationService by mocking EmailSender.
So isn’t the testability concern primarily about how the dependency is provided (hard-coded vs injected), rather than whether it’s an interface?
