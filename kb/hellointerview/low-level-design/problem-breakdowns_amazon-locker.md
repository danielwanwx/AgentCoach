...
Bad Solution: Compute availability from access tokens

Good Solution: Index available compartments by size (optimal performance)

Great Solution: Track occupancy on Compartment
...
Why is the Locker class considered to be violating the Single Responsibility Principle (SRP)? Currently, the Locker class is:

Managing compartments (availability, assignment, occupancy)
Managing access tokens (generation, storage, expiry)

Why can’t we follow a Repository + Service pattern instead?

A CompartmentRepository and CompartmentService to handle locker compartments
An AccessTokenRepository and AccessTokenService to handle access tokens
A LockerSystem (or LockerOrchestrationService) that acts as the entry point and coordinates these services

Wouldn’t this separation improve modularity?
2) Why is concurrency not handled while assigning lockers?
In a real-world locker system, multiple users could try to deposit packages at the same time.
Shouldn’t concurrency control (e.g., synchronization, locks, or atomic operations) be part of the locker assignment logic to avoid race conditions and double allocation?

Isn’t handling such concurrency an expected consideration in Low-Level Design (LLD)?
...
i think for the reposiory + service pattern, it is not feasible for lld interview where you are expected to code it in 45 mins to 1 hour. Yes, for machine coding it is expected.
