...
Limiter.py
LimiterFactory.py
TokenBucketLimiter.py
SlidingWindowLogLimiter.py
Python
Language
...
Kudos to the team! Well Explained in detail.

In real time, though rate limit algorithm (TokenBucket/Sliding Window) remains same per endpoint. client rate limit configs would vary for each client.
How about moving the Rule Config (Eg: In case of token bucket, capacity/refillRate configs inside bucket)? By this, we can accomodate the varying  rate limit rules per client.
...
You're talking about client specific configs per endpoint, yah? For an interview (certainly as we've defined it in this breakdown) it's out of scope. But if this was something you wanted then, you'd probably add a tier/plan lookup step before delegating to the limiter. Something like clientTier = getClientTier(clientId) then route to the appropriate limiter instance for that tier. So instead of limiters[endpoint] you'd have limiters[(endpoint, tier)]. Moving config into each individual bucket would work but means you can't share limiter instances across clients in the same tier (ie premium vs free user)
