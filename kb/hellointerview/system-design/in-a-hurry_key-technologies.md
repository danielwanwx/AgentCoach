...
Agree, not really system design interview topics.
Still common in real projects though, especially outside big tech, and pretty hard to grok properly.
...
While Redlock is great for best-effort locking, it may not be suitable for highly critical systems where strong consistency is required.
If data corruption is unacceptable (e.g., bank account transfers, stock trading), a consensus-based system like Zookeeper (ZAB), etcd (Raft), or Google Spanner (Paxos) is a better choice.
...
Alice Request -> Request Lock for Alice Row and Bob Row in that order.
Bob Request -> Request Lock on Bob Row and Alice Row in that order.

During simultaneous locking requests.
Alice -> obtained 1st lock -> Alice Row is locked.
Bob -> obtained 1st lock -> Bob row is locked.

Alice -> waiting for 2nd lock -> Bob row is already locked by Bob.
Bob -> waiting for 2nd lock -> Alice row is already locked by Alice.

This is deadlock.

Now this could easliy be avoided if both have followed acquiring locks in same order.

Alice -> acquire lock on Alice row & Bob row in that order.
Bob -> acquire lock on Alice row and Bob row in that order.

Now only 1 would be able to acquire the first lock. Let's say Bob acquires the 1st lock.

Bob -> acquired 1st lock -> Alice row is locked by bob.
Alice -> can't acquire lock -> Wait / retry / error out.

Bob -> acquires 2nd lock -> Bob row is locked by bob.
Alice -> still waiting / retry / error out.
