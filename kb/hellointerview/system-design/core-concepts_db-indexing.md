...
https://youtu.be/yvBR71D0nAQ?t=830
...
The core issue with a composite index (lat, lng) is the shattering of spatial locality.

Example: Consider two restaurants in Bellevue, WA. One is at (47.61, -122.19) and its neighbor is at (47.62, -122.19). In the real world, they are right next to each other. However, in a B-tree, they are separated by every single restaurant in the world that sits between Latitude 47.61 and 47.62.

Because the index is strictly sorted by lat first, these 'nearby' points end up being physically fragmented across distant disk pages. This forces the DB to perform a massive 'band scan' or thousands of random I/O seeks to stitch the results together. The I/O penalty is enormous compared to a Geohash, which clusters these points into the same disk block.

4

Reply
M
MelodicAmethystNarwhal378
Premium
• 3 months ago

The issue is that a B-Tree composite index turns a 2D "box" query into a massive 1D "band" scan.
The "Band" Problem (Composite Index): A B-Tree sorts strictly by the first column (lat) before the second. If you query for restaurants near New York (Lat: 40), the database must scan every row where Lat=40. This means it reads restaurants in Madrid, Beijing, and Tokyo just to filter them out by Longitude later. You scan a horizontal strip across the entire planet.
The "Box" Solution (Geohash): Geohashing interleaves the bits of lat/long into a single string. Points that are physically close share the same prefix (e.g., dr5r...). The database can jump to that prefix and scan only the relevant "box," skipping the rest of the world entirely.

TL;DR: Composite indexes scan the whole latitude line; Geohashes scan only the local neighborhood.
