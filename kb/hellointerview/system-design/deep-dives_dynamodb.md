...
Two INCREDIBLY useful behind-the-scenes videos for DynamoDB are re:invent 2024 talks from our Sr. PE, Amrith: https://www.youtube.com/watch?v=csvPepC6tKk and https://www.youtube.com/watch?v=Qzs8mU5dgx4
...
The biggest difference is that Cassandra is write optimized (LSM) while Dynamo is read optimized (BTree).
Cassandra's support for 2nd index is not very good.
Dynamo has support for CDC (change data capture) while (I think) Cassandra does not.
...
Is this correct?
Cassandra would be preferred over DynamoDB when the query pattern is going to require columnar operations, e.g. real-time analytics.
...
Did a small css trick to reveal that:
https://imgur.com/a/RVBSDOz
...
https://i.imgur.com/Xz4MAhS.png here's a picture of the issue.
...
A huge doubt here.
Is the 1000 WCU and 3000 RCU, per partition key value (or)
...
probably physical https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/bp-partition-key-design.html
