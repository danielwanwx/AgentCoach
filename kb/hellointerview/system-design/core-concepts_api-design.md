...
from gemini , but I trust hello interview more
As an API design expert, let's break down the two options for a Ticketmaster-like booking API and determine which is "more correct" and why.
...
POST /bookings/:eventId is clearly a non-starter. The resource should contain its own resource id, not other resources' id. That being said, valid REST API could have:
/v1/events/:eventId/bookings - for creating booking  - returns a bookId.
/v1/bookings/:bookingId - for viewing bookings
