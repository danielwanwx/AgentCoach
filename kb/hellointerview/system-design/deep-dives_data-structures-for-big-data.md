...
Makes sense. But what you described is key idea that should have been metnioned in this article
...
m is the number of different values a hash function returns or the "width" of the stamps (2 in the examples). k is the number of hash functions we have or the "height" of the stamps.
The more values the hash functions return the "wider the spread", so less chance for collision.
It's explained a bit more in the Count-Min Sketch section:

Width w: Controls collision probability. More width = fewer collisions = better accuracy
Depth d: Controls confidence in the estimate. More depth = higher confidence

w is what was referred to as m in the Bloom filter section and d is what k was.
...
In the Bloom Filter section, the image seems to imply we have an array of bit arrays...why is that? As far as I knew a basic Bloom filter is just a bit array and the output of the hash function is the index in that array.
