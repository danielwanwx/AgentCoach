...
A lot of other blogs/vlogs mention that for HTTPS/Websocket connections we should use Layer 7 load balancers. Moreover, for persistent connections or sticky sessions too, they suggest using Layer 7 load balancers as you can inspect cookies, etc.
...
Do we have 2 websocket connections? One is between the client and LB and another is between LB and server ?
...
Type: Modern transport protocol built on UDP
How it works:Developed by Google; now basis for HTTP/3
Uses UDP’s speed but adds features like:
Reliability & encryption (like TCP + TLS combined)
Faster connection establishment (0-RTT)
Multiplexing streams → prevents head-of-line blocking issues in TCP
Use cases: YouTube, Google services, HTTP/3 web browsing
Pros: Fast, secure, reliable, low latency
