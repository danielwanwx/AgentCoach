...
Hi Stefan, this is a well written guide. For system design interviews can we have few questions which actually make use of these vector databases at scale so that we can get an idea of how to go about designing it
...
Hey Stefan, I like to print and read , could you please give print button if possible.
...
In semantic search, can an entire document be stored as a vector embedding? or we store the metadata of the document like title, tags, size etc.
...
Both, depending on the document size. For short documents (a few paragraphs), you can embed the entire text directly into a single vector. For longer documents, you'd typically chunk them into smaller pieces (paragraphs, sliding windows of sentences, etc.) and embed each chunk separately. This is the standard RAG pattern. Metadata like title and tags can also be embedded, or more commonly stored as filterable metadata fields alongside the vector. The embedding captures semantic meaning of the text content, not the metadata — so you'd embed the actual text you want to be able to search against semantically.
...
How are concurrent writes handled in single node vs cluster of nodes ? What about wal + batch writes + fsync for these databases ? Are there any numbers I can use to understand at what stress level these databases break down - either in connections or write ops or read ops ?

What about encoding of these many field values or compression rates ?  What about any similarities between SIMD in spark off heap execution vs these vector engines ?
...
Thanks for the deep dive Stefan! When you say "minimizing the dot product," does that mean you want the embeddings of items bought together to be farther apart? Or is the goal to make them closer together in vector space?

"If your embedding model is trained on a specific task, you'll use that (e.g. minimizing the dot product between two items that are bought together, that's your distance metric)."
